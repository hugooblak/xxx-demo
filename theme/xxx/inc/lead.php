 ...
 * or photo. They call xx_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional ..
 *
 * @package .
 */
 ...
 * @return string
 */
function xx_sample_note() {
	return xx_lead( 'ar_riktig' ) ? '' : ' <span class="xx-sample">(platshållare)</span>';
}
 ...
 * @return string File name for xx_lead_img().
 */
function xx_lead_bild( $i ) {
	$bilder = (array) xx_lead( 'bilder', array() );
 ...
 * @return string
 */
function xx_ring_label() {
	$namn = xx_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, xx_lead( 'telefon' ) ) : 'Ring ' . xx_lead( 'telefon' );
}
