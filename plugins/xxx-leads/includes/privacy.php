...
add_filter(
	...
	function ( $exporters ) {
		$exporters['xxx-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'xxx-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'xxx-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'xxx-leads',
						'group_label' => __( 'Quote requests', 'xxx-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['xxx-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'xxx-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
