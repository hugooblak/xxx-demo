...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'xxx-leads' ),
					'singular_name' => __( 'Lead', 'xxx-leads' ),
					'edit_item'     => __( 'Förfrågan', 'xxx-leads' ),
					'search_items'  => __( 'Search leads', 'xxx-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'xxx-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'xxx-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'xxx-leads' ),
			'npl_phone'    => __( 'Phone', 'xxx-leads' ),
			'npl_service'  => __( 'Needs', 'xxx-leads' ),
			'npl_timeline' => __( 'When', 'xxx-leads' ),
			'npl_zip'      => __( 'ZIP', 'xxx-leads' ),
			'npl_source'   => __( 'Source', 'xxx-leads' ),
			'npl_status'   => __( 'Status', 'xxx-leads' ),
			'date'         => __( 'Received', 'xxx-leads' ),
		);
	}
);
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'xxx-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'xxx-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'xxx-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'xxx-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'xxx-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'xxx-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'xxx-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'xxx-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'xxx-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'xxx-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'xxx-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
